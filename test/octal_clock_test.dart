...
void main() {
  group('OctalDateTime', () {
    group('::new', () {
      test('gets midnight correct', () {
        _expectDate(
            OctalDateTime(2017, 10, 31), 2017, 10, 31, 0, 0, 0, 0, 0, false);
      });

      test('gets midnigth correct utc', () {
        _expectDate(
            OctalDateTime.utc(2017, 10, 31), 2017, 10, 31, 0, 0, 0, 0, 0, true);
      });

      test('handles milliseconds correctly', () {
        _expectDate(OctalDateTime(2017, 1, 1, 0, 0, 0, 777, 777), 2017, 1, 1, 0,
            0, 0, 777, 777, false);
      });

      test('gets MS correctly', () {
        _expectDate(OctalDateTime.utc(2017, 10, 31, 7), 2017, 10, 31, 7, 0, 0,
            0, 0, true);
        _expectDate(OctalDateTime(2017, 10, 31, 10), 2017, 10, 31, 10, 0, 0, 0,
            0, false);
        _expectDate(OctalDateTime.utc(2017, 10, 31, 11), 2017, 10, 31, 11, 0, 0,
            0, 0, true);
      });

      test('gets DS correctly', () {
        _expectDate(OctalDateTime(2017, 10, 31, 17, 77, 77, 777, 777), 2017, 10,
            31, 17, 77, 77, 777, 777, false);
        _expectDate(OctalDateTime.utc(2017, 10, 31, 20), 2017, 10, 31, 20, 0, 0,
            0, 0, true);
        _expectDate(OctalDateTime(2017, 10, 31, 21), 2017, 10, 31, 21, 0, 0, 0,
            0, false);
      });

      test('gets VS correctly', () {
        _expectDate(OctalDateTime.utc(2017, 10, 31, 27), 2017, 10, 31, 27, 0, 0,
            0, 0, true);
        _expectDate(
            OctalDateTime(2017, 10, 31, 30), 2017, 11, 1, 0, 0, 0, 0, 0, false);
      });

      test('errors on invalid octal numbers', () {
        expect(
          () => OctalDateTime(2017, 10, 31, 29),
          throwsA(allOf(
              isArgumentError,
              predicate((dynamic e) =>
        ...
        expect(
          () => OctalDateTime(2017, 10, 31, 27, 18),
          throwsA(allOf(
              isArgumentError,
              predicate((dynamic e) =>
        ...
        expect(
          () => OctalDateTime(2017, 10, 31, 27, 15, 68),
          throwsA(allOf(
              isArgumentError,
              predicate((dynamic e) =>
        ...
        expect(
          () => OctalDateTime(2017, 10, 31, 27, 15, 45, 182),
          throwsA(allOf(
              isArgumentError,
              predicate((dynamic e) =>
                  e.message ==
        ...
        expect(
          () => OctalDateTime(2017, 10, 31, 27, 15, 45, 74, 935),
          throwsA(allOf(
              isArgumentError,
              predicate((dynamic e) =>
                  e.message ==
    ...
    group('::fromDateTime', () {
      test('converts correctly', () {
        final dt = DateTime.utc(2017, 10, 31, 8, 50, 37, 450, 268);
        _expectDate(OctalDateTime.fromDateTime(dt), 2017, 10, 31, 10, 65, 77,
            743, 211, true);
      });
    });
      ...
      test('errors on invalid octal', () {
        expect(
          ...
          throwsA(allOf(
            isArgumentError,
            predicate((dynamic e) =>
                e.message == 'hour (18) is not a valid octal number.'),
          )),
        );
        expect(
          ...
          throwsA(allOf(
            isArgumentError,
            predicate((dynamic e) =>
                e.message == 'minute (38) is not a valid octal number.'),
          )),
        );
        expect(
          ...
          throwsA(allOf(
            isArgumentError,
            predicate((dynamic e) =>
                e.message == 'second (29) is not a valid octal number.'),
          )),
        );
        expect(
  ...
  group('OctalDuration', () {
    ...
    test('handles subtracting durations', () {
      var first = OctalDuration(
          ...
          milliseconds: 5,
          microseconds: 6);
      var second = OctalDuration(days: 1, seconds: 4);
    ...
    test('handles multiplying durations', () {
      var duration = OctalDuration(days: 1, seconds: 1);
      _expectDuration(
          duration * 2, 2, 60, 6000, 600002, 600002000, 600002000000);

      duration = OctalDuration(
          days: 1,
          hours: 2,
          minutes: 3,
          seconds: 4,
    ...
    test('handles dividing durations', () {
      var duration = OctalDuration(days: 2, seconds: 2, microseconds: 3);
      _expectDuration(
          duration ~/ 2, 1, 30, 3000, 300001, 300001000, 300001000001);
    });

    test('handle comparisons', () {
      var oneday = OctalDuration(days: 1);
      var twoday = OctalDuration(days: 2);
      var thirtyhours = OctalDuration(hours: 30);
      var sixtyhours = OctalDuration(hours: 60);
    ...
    test('::fromDuration', () {
      var duration = OctalDuration.fromDuration(Duration(days: 1, hours: 1));
      _expectDuration(duration, 1, 31, 3100, 310000, 310000000, 310000000000);

      duration =
          OctalDuration.fromDuration(Duration(days: 1, hours: 1, minutes: 1));
      _expectDuration(duration, 1, 31, 3101, 310104, 310104211, 310104211000);

      duration = OctalDuration.fromDuration(
      ...
      duration = OctalDuration.fromDuration(Duration(
          days: 1,
          hours: 8,
          minutes: 10,
          seconds: 59,
    ...
    test('::toDuration', () {
      var duration = OctalDuration(days: 1, hours: 1);
      _expectDuration(
          duration.toDuration(), 1, 25, 1500, 90000, 90000000, 90000000000);

      duration = OctalDuration(days: 1, hours: 1, minutes: 1);
      _expectDuration(
          duration.toDuration(), 1, 25, 1500, 90056, 90056250, 90056250000);

      duration = OctalDuration(days: 1, hours: 1, minutes: 1, seconds: 1);
      _expectDuration(
  ...
  group('OctalDateTime manipulation with OctalDuration', () {
    test('adds durations to OctalDateTimes', () {
      ...
      odt = odt.add(OctalDuration(microseconds: 6));
